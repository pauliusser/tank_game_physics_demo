...
public class VictoryScreen : MonoBehaviour
{
    ...
    public void InitializeUI()
    {
        ...
        if (uiDocument == null)
        {
            ...
            if (uiDocument == null)
            {
                if (debug) Debug.LogError("No UIDocument found!");
                return;
            }
        }
        ...
        {
            if (debug) Debug.LogWarning("GUIDocument is not enabled");
            return;
        }
        
        if (uiDocument.rootVisualElement == null)
        {
            if (debug) Debug.LogError("rootVisualElement is null - UI might not be ready");
            return;
        }
        ...
        {
            if (debug) Debug.LogError("Could not find 'restart-btn'");
        }
        ...
        {
            if (debug) Debug.LogError("Could not find 'MainMenu-btn'");
        }
        
        finalScoreLabel = root.Q<Label>("finalScore-label");
        if (finalScoreLabel == null)
        {
            if (debug) Debug.LogError("Could not find 'finalScore-label'");
        }
        
        uiInitialized = true;
        if (debug) Debug.Log("UI initialized successfully");
    ...
    void UpdateFinalScore(int finSc)
    {
        ...
        {
            if (debug) Debug.Log($"UI not ready, storing score {finSc}");
            pendingScore = finSc;
            return;
        }
        
        finalScoreLabel.text = $"Final Score: {finSc:D4}";
        if (debug) Debug.Log($"Updated final score to {finSc:D4}");
    }
